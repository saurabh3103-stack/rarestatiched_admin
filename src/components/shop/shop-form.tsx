...
import { HttpClient } from '@/data/client/http-client'; // Import your HttpClient
import { API_ENDPOINTS } from '@/data/client/api-endpoints'; // Import your API endpoints
// const socialIcon = [
//   {
//     value: 'FacebookIcon',
//     label: 'Facebook',
...
const ShopForm = ({ initialValues }: { initialValues?: Shop }) => {
  ...
  const [totalShops, setTotalShops] = useState(0); // Add state for totalShops
  useEffect(() => {
    // Function to call the API and set data
    const fetchAnalyticsData = async () => {
      try {
        const data = await HttpClient.get<any>(API_ENDPOINTS.ANALYTICS); // Call API using HttpClient
        setTotalShops(data.totalShops); // Print out the API data in the console
        // Store the data in state
      } catch (error) {
        console.error('Error fetching analytics data:', error); // Handle any errors
      }
    };

    fetchAnalyticsData(); // Call the function
  ...
  //       'https://fun2sh.deificindia.com/shops?search=is_active:1&is_active=1&language=en&searchJoin=and&limit=30&sortedBy=DESC&orderBy=created_at'
  //     );
  ...
  // Access total number of shop IDs
  //       const totalShopsCount = data.data.length;
  //       setShopData(data); // Save shop data
  //       setTotalShops(totalShopsCount); // Save total number of shops in state
  //       console.log('Total Shops:', totalShopsCount); // Print total number of shop IDs
  //     } catch (error) {
